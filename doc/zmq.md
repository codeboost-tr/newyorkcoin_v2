...
The NewYorkCoin Core daemon can be configured to act as a trusted "border
router", implementing the NewYorkCoin wire protocol and relay, making
...
The ZeroMQ feature in NewYorkCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building nycd:
...
For instance:
    ...
    $ nycd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/nycd.tx.raw \
...
These options can also be provided in newyorkcoin.conf.
...
From the perspective of nycd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into nycd directly. Furthermore, no information is
...
using. NewYorkCoind appends an up-counting sequence number to each
